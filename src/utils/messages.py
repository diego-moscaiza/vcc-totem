...
def generar_mensaje_personalizado(estado, datos=None, mensaje_error=None):
    ...
    elif estado == "dni_invalido" or (
        mensaje_error
        and (
            "no encontrado" in mensaje_error.lower()
            or "no califica" in mensaje_error.lower()
            or "no tiene campaña" in mensaje_error.lower()
        )
    ):
...
def mostrar_resultado(dni, data, estado="success", mensaje_api=None):
    ...
    try:
        ...
        return (
            ...
            estado_dni,
            False,
        )  # ← Agregado False para tiene_oferta en caso de error
