    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if mensaje:  # Solo procesar si hay mensaje
                    ...
                    if not data.get("valid") and (
                        "no encontrado" in mensaje or "no existe" in mensaje
                    ):
            ...
            logger.debug(
                f"Timeout en consulta rápida para DNI {dni}, intentando consulta completa"
            )
        ...
        if response.status_code == 200:
            ...
            if data.get("valid"):
                ...
                # Agregar clave segmento a la respuesta FNB
                respuesta_data = data["data"]
                respuesta_data["segmento"] = "fnb"
                return respuesta_data, "success", None
            else:
        ...
        logger.error(
            f"Tiempo de espera agotado ({TIMEOUT} segundos) consultando DNI {dni}"
        )
        return (
            None,
            "timeout",
            f"La consulta excedió el tiempo máximo de espera de {TIMEOUT} segundos. Por favor, inténtelo nuevamente.",
        )
